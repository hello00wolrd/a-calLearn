...
//     // 类的析构方法：当类的实例使用完并从内存中释放时，将会触发调用该方法,所以不需要关闭数据库,在析构时就会关闭
//     //以两个下划线开头的叫做魔术方法,不需要调用自动执行
//     function __destruct() {
//         $conn = $this->create_connection();
//         mysqli_close($conn);
//     }
...
class DB {
    // 为DB类定义数据库连接的必要属性,还是建议使用私有的sql值
    ...
    // 当类(实例)进行序列化时自动调用，并且返回一个数组，包含要实例化的类属性
    //当单单进行序列化时,并不会调用构造方法,此类里面没有与数据库建立连接
    function __sleep() {
        echo "DB类正在进行序列化. <br/>";
